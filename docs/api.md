...
如果最小请求正常，但 Codex / Claude Code 等客户端仍失败，请查看错误中的 `Prompt diagnostics`。这些客户端可能携带隐藏系统提示和历史上下文，实际 prompt 会明显长于可见消息。Tabbit2API 会在超过 19000 字符时自动压缩 system、developer 和旧历史上下文，并把发送长度控制在 Tabbit 实测约 20500 字符的上限以内；最新用户消息不会被截断。若安全压缩后仍然超限，接口会返回 `invalid_request`，此时需要减少当前用户输入、附件元数据、工具 schema 或最近上下文。
